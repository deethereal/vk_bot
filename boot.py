...
COMAND=False
groupID = 178950051
token = 'AAAAAAAAAA' # Здесь ввести token сообщества (не удаляя апострофы)
roll=False
mute_mode=False
votekick = False
votekickTime=0
       ...
       # print(event.object['message']['action'])
        ...
        if message_text=='/help':
            ...
            if votekick:
                ...
                if not COMAND:
                    if findIII(message_text) and not comands["ы"]:
                         send(findIII(message_text),event.object['message']['peer_id'] )
                    if findWord(message_text, 'хуй') and not comands["хуй"]:
                    ...
                    if findWord(message_text,'доска') or findWord(message_text,'ирка'):
                    ...
                    if findWordInList(message_text,imposter) and not comands["пидор"]:
                        sendphoto('', event.object['message']['peer_id'], 'photo-178950051_4572391'+str(67+random.randint(0, 7)))
                    if findWordInList(message_text,torch) and not comands["кальян"]:
                    ...
                    if findWord(message_text,'хачу') and not comands["хочу"]:
                        try:
                            ha4u(message_text)
                        except ValueError:
                            continue
                    if findWord(message_text,'хочу') and not comands["хачу"]:
                        try:
                            ho4u(message_text)
                        except ValueError:
                            continue
                    if message_text=='!отладка':
                        ...
                        send(printdic(comands),event.object['message']['peer_id'])
                    ...
                    elif (findWord(message_text, 'мама') or findWord(message_text, 'мамка') or findWord(message_text, 'мамку') or findWord(message_text, 'маму') or findWord(message_text, 'маман') or findWord(message_text, 'маме') or findWord(message_text, 'мамке')) and not comands["мама"]:
