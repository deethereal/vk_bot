            ...
            if message_text.startswith("/дать"):
                ...
                if len(words) == 3:
                    if (words[1] == "владу") or (words[1] == "владику") or (words[1] == "владиславу"):
                        ...
                        outcome_text = calculate_social_rating(str(event.object["message"]["from_id"]), dicts["Matvey_inc_dict"]["green"][0], int(words[2]), dicts)
                    elif (words[1] == "саше") or (words[1] == "александру"):
                        outcome_text = calculate_social_rating(str(event.object["message"]["from_id"]), dicts["Matvey_inc_dict"]["sasha"][0], int(words[2]), dicts)
                    elif words[1] == "никите":
                        outcome_text = calculate_social_rating(str(event.object["message"]["from_id"]), dicts["Matvey_inc_dict"]["orange"][0], int(words[2]), dicts)
                    elif (words[1] == "коле") or (words[1] == "николаю"):
                        outcome_text = calculate_social_rating(str(event.object["message"]["from_id"]), dicts["Matvey_inc_dict"]["yellow"][0], int(words[2]), dicts)
                    elif (words[1] == "моте") or (words[1] == "матвею"):
                        outcome_text = calculate_social_rating(str(event.object["message"]["from_id"]), dicts["Matvey_inc_dict"]["red"][0], int(words[2]), dicts)
